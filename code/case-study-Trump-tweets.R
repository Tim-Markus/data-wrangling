...
path <- 'resources/tweets.json'
trump_tweets <- map(2009:2017, ~sprintf(path, .x)) %>%
  map_df(jsonlite::fromJSON, simplifyDataFrame = TRUE) %>%
  filter(!isRetweet & !str_detect(text, '^"')) %>%
  mutate(created_at = parse_date_time(created_at, orders = "a b! d! H!:M!:S! z!* Y!", tz="EST")) 

trump_tweets <- jsonlite::fromJSON("resources/tweets.json", simplifyDataFrame = TRUE) %>%
  as_tibble() %>%
...
sort(unique(get_sentiments("afinn")$value))
